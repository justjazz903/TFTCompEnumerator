...
def reduce_combination(population, sample, step, include, size, result):
    ...
    if current_size <= size:
        ...
        return
    reduce_combination(population[0: -1], sample, step, include, size, result)
    include.append(population[-1])
    reduce_combination(population[0: -1], sample - step, step, include, size, result)
...
def reduce_combination_set6(population, sample, include, size, result):
    ...
    for i in range(len(colossus) + 1):
        ...
        for comb in colossus_comb:
            reduce_combination(population, sample - step * i, step, list(comb), size, result)
